...
const finishTracking = async (data?: ITrackingInformation) => {
	analyticsLoggingService.logData({ message: `analyitcs-broker-process finish tracking started, sentFinishMsg: ${sentFinishMsg}, receivedFinishMsg: ${receivedFinishMsg}` });
	...
	if (!sentFinishMsg) {
		...
		const action = async () => {
			await trackingQueue;
			analyticsLoggingService.logData({ message: `analyitcs-broker-process tracking finished` });
			process.disconnect();
		};
...
process.on("message", async (data: ITrackingInformation) => {
	analyticsLoggingService.logData({ message: `analyitcs-broker-process received message of type: ${data.type}` });
...
process.on("disconnect", async () => {
	analyticsLoggingService.logData({ message: "analyitcs-broker-process received process.disconnect event" });
	await finishTracking();
});

analyticsLoggingService.logData({ message: `analyitcs-broker-process will send ${AnalyticsMessages.BrokerReadyToReceive} message` });
